<div class="login-options">
    <div class="login-options-title">
        <p>{{ __('Register with social networks') }}</p>
    </div>

    @if (setting('social_login_style', 'default') === 'basic')
        <ul class="social-login-basic">
            @foreach (\Botble\SocialLogin\Facades\SocialService::getProviderKeys() as $item)
                @continue(!\Botble\SocialLogin\Facades\SocialService::getProviderEnabled($item))
                <li>
                    <a href="{{ route('auth.social', array_merge([$item], $params)) }}"
                        class="social-login {{ $item }}-login">
                        @php $iconName = $item === 'linkedin-openid' ? 'linkedin' : $item; @endphp
                        <img src="{{ asset('vendor/core/plugins/social-login/images/icons/logo-' . $iconName . '.svg') }}"
    ...
    @else
        <ul @class(['social-icons', 'social-login-lg' => setting('social_login_style', 'default') === 'default'])>
            @foreach (\Botble\SocialLogin\Facades\SocialService::getProviderKeys() as $item)
                @continue(!\Botble\SocialLogin\Facades\SocialService::getProviderEnabled($item))
                {!! apply_filters('social_login_' . $item . '_render',
