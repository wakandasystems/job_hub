...
@section('content')
    ...
    <x-core::card>
        <x-core::card.header>
            <x-core::card.title>Job Alert Package Orders</x-core::card.title>
        </x-core::card.header>
        <x-core::card.body>
            <form method="GET" action="{{ route('job-alert-orders.index') }}" class="row g-2 mb-3">
                <div class="col-md-5">
                ...
                <div class="col-md-3">
                    <select class="form-select" name="status">
                        ...
                        @foreach(\Botble\JobBoard\Models\JobAlertOrder::statuses() as $value => $label)
                ...
                <div class="col-md-4">
                    ...
                    <a class="btn btn-outline-secondary" href="{{ route('job-alert-orders.index') }}">Reset</a>
                </div>
            </form>
            ...
            <div class="table-responsive">
                <table class="table table-vcenter table-striped">
                    ...
                    <tbody>
                                ...
                                <td class="text-end">
                                    @if($order->status === 'pending')
                                        <button type="button" class="btn btn-sm btn-success"
                                            ...
                                            data-action="{{ route('job-alert-orders.approve', $order) }}"
                                            data-label="{{ $order->account?->name ?? 'this order' }} — {{ $order->package?->name ?? '' }}">
                                            Approve
                                        </button>
                                        <button type="button" class="btn btn-sm btn-danger"
                                            data-bs-toggle="modal" data-bs-target="#rejectModal"
                                            data-action="{{ route('job-alert-orders.reject', $order) }}"
                                            data-label="{{ $order->account?->name ?? 'this order' }} — {{ $order->package?->name ?? '' }}">
                                            Reject
                                        </button>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted">No job alert orders found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-body text-center py-4 px-4">
                    ...
                    <p class="text-muted small mb-4" id="approveModalLabel">Quota will be credited to the customer immediately.</p>
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-body text-center py-4 px-4">
                    ...
                    <p class="text-muted small mb-3" id="rejectModalLabel">The customer will not receive any quota.</p>
                    <div class="d-flex gap-2 justify-content-center">
                        <button type="button" class="btn btn-outline-secondary px-4" data-bs-dismiss="modal">Cancel</button>
