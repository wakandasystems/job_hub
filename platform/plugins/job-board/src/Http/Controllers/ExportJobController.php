...
use Botble\JobBoard\Exporters\JobExporter;
use Illuminate\Support\Facades\DB;
use Symfony\Component\HttpFoundation\StreamedResponse;

class ExportJobController extends ExportController
{
    protected function getExporter(): Exporter
    {
        $exporter = JobExporter::make();
        ...
        if (request()->has('start_date') && request()->input('start_date') !== '') {
            $exporter->setStartDate(request()->input('start_date'));
        }

        if (request()->has('end_date') && request()->input('end_date') !== '') {
            $exporter->setEndDate(request()->input('end_date'));
        }
    ...
    public function store(ExportRequest $request)
    {
        $request->validate([
            ...
            'status' => ['nullable', 'string', 'in:' . implode(',', BaseStatusEnum::values()) . ',closed'],
